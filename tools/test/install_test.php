...
class Install_test extends Spark_test_case
{
    ...
    {
        $clines = $this->capture_buffer_lines(
            function ($cli) {
                $cli->execute('install', array('-v1.0', 'example-spark'));
            }
        );
        $success = (bool)(strpos(
                end($clines), chr(27) . '[1;36m[ SPARK ]' .
                chr(27) . '[0m Spark installed'
            ) === 0);
    ...
    function test_install_with_invalid_spark()
    {
        ...
        $success = (bool)(strpos(
                end($clines), chr(27) . '[1;31m[ ERROR ]' .
                chr(27) . '[0m Unable to find spark'
            ) === 0);
        Spark_utils::remove_full_directory(SPARK_PATH . '/example-spark');
        $this->assertEquals(true, $success);
    }
    ...
    function test_install_with_invalid_spark_version()
    {
        ...
        $success = (bool)(strpos(
                reset($clines), chr(27) . '[1;31m[ ERROR ]' .
                chr(27) . '[0m Uh-oh!'
            ) === 0);
        Spark_utils::remove_full_directory(SPARK_PATH . '/example-spark');
        $this->assertEquals(true, $success);
